...
// solved 12/8/24
// comments: good run! knowing set and pair helped a lot, got through the math fine
    ...
    for (auto it1 = s.begin(); it1 != s.end(); ++it1) { 
      for (auto it2 = s.begin(); it2 != s.end(); ++it2) { 
        if (it1->F == it2->F && *it1 != *it2){
          ...
          if(left.F>=0 && left.F<m && left.S>0 && left.S<=n){
            vret[left.S-1][left.F] = '#';
          }
          if(right.F>=0 && right.F<m && right.S>0 && right.S<=n){
